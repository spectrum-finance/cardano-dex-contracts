...
checkPool = testGroup "CheckPoolContract"
  ...
  , HH.testProperty "pool_swap_incorrect_swap_time" poolSwapIncorrectTime
...
successPoolSwap = property $ do
  ...
  poolTxRef <- forAll genTxOutRef
  let
    (pcfg, pdh) = genPConfig x y nft lq 995 [] (POSIXTime 0)
...
poolSwapIncorrectTime :: Property
poolSwapIncorrectTime = property $ do
  let (x, y, nft, lq) = genAssetClasses
  pkh             <- forAll genPkh
  orderTxRef      <- forAll genTxOutRef
  let
  ...
  poolTxRef <- forAll genTxOutRef
  let
    (pcfg, pdh) = genPConfig x y nft lq 995 [] (POSIXTime 1000000)
    poolTxIn    = genPTxIn poolTxRef pdh x 10 y 10 lq 9223372036854775797 nft 1 1000000000
    poolTxOut   = genPTxOut pdh x 20 y 6 lq 9223372036854775797 nft 1 3000000
...
poolSwapRedeemerIncorrectIx = property $ do
  ...
  poolTxRef <- forAll genTxOutRef
  let
    (pcfg, pdh) = genPConfig x y nft lq 1000 [] (POSIXTime 0)
...
successPoolChangeStakePartCorrectMinting = property $ do
  ...
  poolTxRef <- forAll genTxOutRef
  let
    (pcfg, pdh) = genPConfig x y nft lq 1 [stakeAdminPkh] (POSIXTime 0)
...
failedPoolChangeStakePartIncorrectMinting = property $ do
  ...
  poolTxRef <- forAll genTxOutRef
  let
    (pcfg, previousPdh) = genPConfig x y nft lq 1 [stakeAdminPkh] (POSIXTime 0)

    (_, newPdh) = genPConfig x y nft lq 1 [] (POSIXTime 0)
...
failedPoolChangeStakePartIncorrectFinalDatum = property $ do
  ...
  let
    (pcfg, pdh) = genPConfig x y nft lq 1 [stakeAdminPkh] (POSIXTime 0)
    (_, incorrectPdh) = genPConfig incorretX incorrectY incorrectNft incorrectlq 1 [stakeAdminPkh] (POSIXTime 0)
    poolTxIn    = genPTxInWithSC poolTxRef previousSc pdh x 10 y 10 lq 9223372036854775797 nft 1 10000
...
poolDepositRedeemerIncorrectIx = property $ do
  ...
  poolTxRef <- forAll genTxOutRef
  let (pcfg, pdh) = genPConfig x y nft lq 1 [] (POSIXTime 0)
